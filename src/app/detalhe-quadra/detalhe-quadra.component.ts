...
export class DetalheQuadraComponent implements OnInit {
  form = this.fb.group({
    ...
    data: [this.getCurrentDate(), Validators.required],
  ...
  async agendar(): Promise<void> {
    if (this.form.valid) {
      ...
      const dataValue = this.formatDate(this.form.value.data || '');

      const nomeQuadra = await this.quadraService.getNomeQuadraById(this.quadraId);
      const valorHora = await this.quadraService.getValorHoraById(this.quadraId);

      const agendamento: Agendamento = {
        id: this.quadraId + '-' + Date.now().toString(),
        idUsuario: this.userEmail,
        idQuadra: this.quadraId,
        nomeQuadra: nomeQuadra ?? 'erro',
        data: dataValue,
        hora: this.form.value.horaInicio + ' - ' + this.form.value.horaFim,
        valorHora: valorHora ?? 0
      };

      try {
        await this.agendamentoService.addAgendamento(agendamento);
        console.log(`Agendado para ${agendamento.data} das ${agendamento.hora}`);
  ...
  private formatDate(date: string | null): string {
    if (!date) {
      return '01/01/0001'; // Retorna a data default quando date for null ou undefined
    }
